...
datospca<-datos_train_numeric[c(1:3)]
...
<p style = "text-align: justify">
Se observa, como la componente 1 (PC1) está dando un peso de 0.41 a log_lot, un peso de 0.66 a log_sqft_living y un peso de 0.62 a bedrooms. La componente 2 (PC2) por otro lado estaría capturando la diferencia entre bedrooms (-0.42) y log_sqft_living (-0.16), frente a log_lot (0.89). Estas conclusiones se extraen de los pesos que cada autovector da a cada variable.
</p>
...
predicciones = predict(model_glm, datos_validacion_limpio[,-15])
predicciones <- ifelse(predicciones > 0.5, yes = 1, no = 0)


tabla_glm_validacion <- table(datos_validacion_limpio$price_categ1, predicciones)
tabla_glm_validacion

#caras
pred_caras_glm_validacion=tabla_glm_validacion[2,2]/(tabla_glm_validacion[2,2]+tabla_glm_validacion[1,2])
rec_caras_glm_validacion=tabla_glm_validacion[2,2]/(tabla_glm_validacion[2,2]+tabla_glm_validacion[2,1])
F_caras_reglog=(2*pred_caras_glm_validacion*rec_caras_glm_validacion)/(pred_caras_glm_validacion+rec_caras_glm_validacion)
cat(c('F1 casas caras: ', F_caras_reglog), '\n')

#baratas
pred_baratas_glm_validacion=tabla_glm_validacion[1,1]/(tabla_glm_validacion[1,1]+tabla_glm_validacion[2,1])
rec_baratas_glm_validacion=tabla_glm_validacion[1,1]/(tabla_glm_validacion[1,1]+tabla_glm_validacion[1,2])
F_baratas_reglog=(2*pred_baratas_glm_validacion*rec_baratas_glm_validacion)/(pred_baratas_glm_validacion+rec_baratas_glm_validacion)
cat(c('F1 casas baratas: ', F_baratas_reglog), '\n')

#F-Medida
F_reglog_validacion= (F_caras_reglog+F_baratas_reglog)/2
cat(c('F1 global: ', F_reglog_validacion), '\n')

accuracy_reglog_validacion = (tabla_glm_validacion[1,1]+tabla_glm_validacion[2,2]) / (tabla_glm_validacion[1,1]+tabla_glm_validacion[1,2]+tabla_glm_validacion[2,1]+tabla_glm_validacion[2,2])
cat(c('Accuracy: ', accuracy_reglog_validacion), '\n')
...
```{r}
predicciones = predict(model_gam_categ, datos_validacion_limpio[,-15])

predicciones <- ifelse(predicciones > 0.5, yes = 1, no = 0)

tabla_gam_validacion <- table(datos_validacion_limpio$price_categ1, predicciones, dnn = c("observaciones", "predicciones"))
tabla_gam_validacion

#caras
pred_caras_gam_validacion=tabla_gam_validacion[2,2]/(tabla_gam_validacion[2,2]+tabla_gam_validacion[1,2])
rec_caras_gam_validacion=tabla_gam_validacion[2,2]/(tabla_gam_validacion[2,2]+tabla_gam_validacion[2,1])
F_caras_gam=(2*pred_caras_gam_validacion*rec_caras_gam_validacion)/(pred_caras_gam_validacion+rec_caras_gam_validacion)
cat(c('F1 casas caras: ', F_caras_gam), '\n')

#baratas
pred_baratas_gam_validacion=tabla_gam_validacion[1,1]/(tabla_gam_validacion[1,1]+tabla_gam_validacion[2,1])
rec_baratas_gam_validacion=tabla_gam_validacion[1,1]/(tabla_gam_validacion[1,1]+tabla_gam_validacion[1,2])
F_baratas_gam=(2*pred_baratas_gam_validacion*rec_baratas_gam_validacion)/(pred_baratas_gam_validacion+rec_baratas_gam_validacion)
cat(c('F1 casas baratas: ', F_baratas_gam), '\n')

#F-Medida
F_gam_validacion= (F_caras_gam+F_baratas_gam)/2
cat(c('F1 global: ', F_gam_validacion), '\n')

accuracy_gam_validacion = (tabla_gam_validacion[1,1]+tabla_gam_validacion[2,2]) / (tabla_gam_validacion[1,1]+tabla_gam_validacion[1,2]+tabla_gam_validacion[2,1]+tabla_gam_validacion[2,2])
cat(c('Accuracy: ', accuracy_gam_validacion), '\n')
...
<p style = "text-align: justify">
De la primer forma obtenemos un valor óptimo de $k=20$, del segundo modo a partir de aproximadamente $k=23$ el error es muy pequeño, y de la forma manual, concluimos que el mejor valor de k en función de la F1-medida es de $k=14$. Finalmente, elegimos el valor de $k=17$, implementamos el modelo y lo evaluamos.
</p>
...
predict_knn_validacion <- knn(scale(datos_train_numeric), scale(datos_validacion_numeric),cl=datos_train_limpio$price_categ1,k=14)

tabla_knn_validacion=table(datos_validacion_limpio$price_categ1, predict_knn_validacion)
...
<p style = "text-align: justify">
El árbol obtenido con los parámetros indicados considera que las variables zona y log_sqft_living son las más importantes para realizar la clasificación. A continuación, se evalúa este modelo y el resultado para la F1-medida y accuracy es de 0.81.
</p>
...
```{r}
datos_decision_tree_validacion <- datos_validacion_limpio
datos_decision_tree_validacion$log_sqft_categ <- cut(datos_decision_tree_validacion$log_sqft_living, breaks = c(0, 3.4, 5), labels = c("P", "G"))

tabla_validacion_arbol=table(obs = datos_decision_tree_validacion$price_categ1, pred = predict(decisiontree_model, datos_decision_tree_validacion, type = "class"))
...
<p style = "text-align: justify">
Pese a que el ajuste de hiperparámetros indica que el valor óptimo de cost es 8, repetimos los cálculos y la F1-global mejora muy poco (aprox. 0.0004). Pasamos de una F1-medida de 0.8686 a una F1-medida de 0.8690.
</p>
...
<p style = "text-align: justify">
El mejor modelo según la métrica de evaluación considerada (F1-medida para las casas caras y las baratas) es el Random Forest y el peor el Decision Tree, si bien es cierto que todos ellos obtienen un resultado aceptable, entre 0.84 y 0.9.
</p>
...
# GAM
# predictions_gam <- predict(model_gam_categ, newdata = datos_train_limpio, type = "response")
